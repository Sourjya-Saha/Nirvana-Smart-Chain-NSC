    ...
    if (contract) {
      ...
      try {
        ...
        if (tokenContract) {
          ...
          // Fix: Compare BigInt values properly
        ...
        setErrorMessage("Validating transaction on blockchain...");
        ...
        setErrorMessage("Transaction validated successfully on blockchain!");
        setHashMismatchError("");
...
const createRazorpayOrder = async () => {
  ...
  try {
    ...
    const options = {
      ...
      // REPLACE THIS HANDLER CODE WITH THE NEW CODE
      handler: function (paymentResponse) {
        ...
        .then(verifyResponse => {
          if (verifyResponse.data.status === 'success') {
            ...
            setErrorMessage("Payment successful! Please click 'Validate After Payment' to complete the transaction on blockchain.");
            setIsPaymentValidated(false); // Reset validation state
          } else {
